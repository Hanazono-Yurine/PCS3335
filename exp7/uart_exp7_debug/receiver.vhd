...
architecture rtl of receiver is
	...
	signal RBR_load : std_logic := '0';
	
	signal RBR_out, RBR_in : std_logic_vector(9 downto 0) := (others => '0');
	signal RBR_L_or_S: std_logic_vector(1 downto 0) := "00";

	signal stateIsfinish, stateIsStopBit, stateIsErroStopBit, stateIsReady : std_logic := '0';
...
begin
	...
	LSR_bit2PE <= '1' when stateIsReady = '1' and receivedParityBit /= parityBit else '0';
	...
	RBR_onlyDataBits <= 
		...
		"0"   & RBR_out(7 downto 1) when LCR_out(1 downto 0) = "10" else
				RBR_out(8 downto 1) when LCR_out(1 downto 0) = "11";

	RBR_L_or_S <= "11" when RBR_load = '1' else "00";

	LSR_bit0 <= '1' when stateIsfinish = '1' else
				'0' when RBR_read = '1' else
				LSR_out(0);
