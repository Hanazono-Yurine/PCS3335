...
architecture arch_baud of baudRateGenerator is
	...
	signal primaryClock : std_logic := '0';
	signal secondaryClock : std_logic := '0';

	signal primaryCounter : unsigned( 15 downto 0 );
	signal primaryCounter_o : std_logic_vector( 15 downto 0 );
	...
	signal secondaryCounter : unsigned( 15 downto 0 );
	signal secondaryCounter_o : std_logic_vector( 15 downto 0 );
	signal secondaryClk : std_logic := '0';
	signal secondaryCounter_reset : std_logic := '0';
	signal secondaryCounter_up : std_logic := '1';
	signal secondaryCounter_out : std_logic := '0';
begin
	primaryCounter <= unsigned( divisor )/18432;
	secondaryCounter <= unsigned( divisor )*100;
	...
	port map (
		clock => primaryClk,
		reset => secondaryCounter_reset,
		enable => '1',
		load => '0',
		up => secondaryCounter_up,
		data_i => ( others => '0' ),
		data_o => secondaryCounter_o
	);
	...
	begin
		...
		if ( reset = '1' ) then
			...
			primaryCounter_up <= '1';
			firstRun := '1';
		elsif ( unsigned(counter1024_o) = primaryClock ) then
	...
	primaryClock <= 1843200/unsigned(divisor);
	secondaryCounter <= divisor;
