...
\begin{document}
	...
	Elaboração de uma UART simplificada. A UART foi dividida em:
	
	\texttt{Baud Rate Generator  (BRG)}, 
	
	\texttt{Transmitter Timing \& Control (TCC)}, 
	
	\texttt{Transmitter Shift Register (TSR)},
	
	\texttt{Line Control Register(LCR)}, 
	
	\texttt{Line Status Register(LSR)}, 
	
	\texttt{Transmitter Holding Register (THR)}. 
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.9\linewidth]{images/RTL1.png}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.9\linewidth]{images/RTL_TCC.png}
		\caption{Diagrama RTL do \texttt{Transmitter Timing \& Control (TCC)}.}
		\label{fig:RTL_TCC}
	\end{figure}
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.9\linewidth]{images/RTL_TSR.png}
		\caption{Diagrama RTL do \texttt{Transmitter Shift Register (TSR)}.}
		\label{fig:RTL_TSR}
	\end{figure}
	...
	A entidade elaborada não apresentou funcionamento correto. 
	Problema: os bits não eram envidados corretamente.
	A sequência de bits envidada não estava correspondendo ao bits carregados no THR.
	O bit paridade também não estava sendo enviado corretamente.
	\par
	Uma possibilidade para a causa do problema é a grande complexidade da FSM utilizada. 
	A dupla irá implementar a uart novamente, dessa vez com utilizando uma FSM mais simples.
	...
	\begin{itemize}
		...
		\begin{table}[H]
			\begin{center}
				\begin{tabular}{|l|l|l|l|}
					...
					0     & 0     & 5 bits           &      FALHA     \\ \hline
					0     & 1     & 6 bits           &    FALHA       \\ \hline
					1     & 0     & 7 bits           &FALHA           \\ \hline
					1     & 1     & 8 bits           &FALHA           \\ \hline
				\end{tabular}
			\end{center}
		\end{table}
		...
		\begin{table}[H]
			\begin{center}
				\begin{tabular}{|l|l|l|l|l|}
					...
					0          & \#         & \#         & SEM bit paridade & FALHA          \\ \hline
					1          & 0          & 0          & Paridade IMPAR   & FALHA          \\ \hline
					1          & 0          & 1          & Paridade PAR     & FALHA          \\ \hline
					1          & 1          & 0          & 1                & FALHA          \\ \hline
					1          & 1          & 1          & 0                & FALHA          \\ \hline
				\end{tabular}
			\end{center}
		...
		\item Quando \texttt{LCR\_bit 6} = 1 $\Rightarrow$ \texttt{serial\_out} = 0. 
		\par Resultado: SUCESSO
